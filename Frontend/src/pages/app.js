import { API_BASE, dbKeyProfilePrefix, dbKeySession, dbKeyEmail, loadSkillsCatalog, skillsByCategory, skillsMap, categoriesList } from "../config/constants.js";
...
function setSession(userId, email) {
  localStorage.setItem(dbKeySession, String(userId));
  localStorage.setItem(dbKeyEmail, email);
  currentUser = userId;
}

function clearSession() {
  localStorage.removeItem(dbKeySession);
  localStorage.removeItem(dbKeyEmail);
  currentUser = null;
  currentUserRecord = null;
...
function renderTopBar() {
  ...
  const email = localStorage.getItem(dbKeyEmail) || "";
  const fullName = `${userRecord.nombre || ""} ${userRecord.apellido || ""}`.trim();
  const visibleName = profile.fullName || fullName || email || String(currentUser);
...
async function ensureOnboardingOrDashboard() {
  ...
  if (!profile || !profile.teachSkills?.length || !profile.learnSkills?.length) {
    ...
    $("fullName").value = (record?.nombre || "") + (record?.apellido ? " " + record.apellido : "");
...
function buildMarketplacePayload(fromProfile = false) {
  ...
  if (fromProfile) {
    const teachNames = profile.teachSkills || [];
    const learnNames = profile.learnSkills || [];
    ...
    mensaje = profile.marketplaceMessage || "";
  } else {
...
async function loadLatestOwnRequest() {
  ...
  try {
    const result = await api(`/message-requests/${currentUser}/outgoing`);
...
async function deleteLatestRequest(requestId = null) {
  ...
  try {
    await api(`/message-requests/${targetRequestId}?user_id=${currentUser}`, {
      method: "DELETE",
    });
...
function confirmDeleteRequest(request) {
  const offered = String(request?.habilidad?.nombre || "").trim() || "(sin habilidad)";
  const requested = String(request?.habilidad_solicitada?.nombre || "").trim() || "(sin habilidad)";
...
function renderLatestRequest() {
  ...
  ownPendingRequests.forEach((request) => {
    ...
    card.innerHTML = `
      <strong>${request.habilidad?.nombre || "-"}</strong>
      <div class="muted">Quiere aprender: ${request.habilidad_solicitada?.nombre || "-"}</div>
      <div class="muted">Estado: ${request.estado}</div>
    `;
