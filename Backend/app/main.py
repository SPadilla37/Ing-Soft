...
def accept_marketplace_request(request_id: str, payload: MarketplaceAcceptRequest) -> dict:
    with SessionLocal() as session:
        ...
        existing_match = get_match_for_users(session, payload.accepter_user_id, target_user_id, request.id)
        if existing_match:
            return {
        ...
        existing_intent = get_match_intent(session, payload.accepter_user_id, target_user_id, request.id)
        ...
        if matched:
            conversation_id = create_match_conversation(session, payload.accepter_user_id, target_user_id, request)
